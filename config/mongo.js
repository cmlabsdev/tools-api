...
class MongoConnection {
    ...
    async connectMongo() {
        ...
        const MONGODB_DB = process.env.MONGODB_DB || 'cmlabs-seo';
    ...
    async findSort(query, sort, limit, collection) {
        return await this.dbClient.collection(collection).find(query).sort(sort).limit(limit).toArray();
    }

    async findSortx(query, sort, limit, collection) {
        return await this.dbClient.collection(collection).find(query,{_id:0,search_volume:1})
            .sort(sort).limit(limit).toArray();
    }

    async findSort2(query, sort, limit, collection) {
        return await this.dbClient.collection(collection).find(query).sort(sort).limit(limit);
    }
